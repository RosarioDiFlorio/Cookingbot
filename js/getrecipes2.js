...
function getPossibleSubstitutions(num){
    $("#chooseSubs").html("");
   offset = offset + num;
   if(offset < 0)offset = 0;
   if(num == 0)offset = 0;
   lang = $("input[name=lang]:checked").val();
    npeople = $('#SUBnpeople').val();
    n = +$('#SUBningredient').val() +1;
    cuisine = $('#cuisine').val().trim();
    diet = $('#diet').val().trim();
    occasion = $('#occasion').val().trim();
    course = $('#course').val().trim();
    ...
    for (i=1;i<n;i++)
    {   
            ingredient = $('#SUBingredient'+i).val().trim();
           // quantity = $('#quantity'+i).val().trim();
            //unit = $('#misurazione'+i).val().trim();
            //mis = $('input[name=mis'+i+']:checked').val().trim();
            input=input+ingredient+";";
...
function add(tipo){
    ...
    n = +$('#SUBningredient').val() +1;
    console.log(n);
    $('#SUBningredient').val(n);
    stringa = "<div class=\"form-group\"><i class=\"fa fa-shopping-cart fa-2x\"></i><div><span class=\"heading\" for=\"comment\">Ingredient "+n+"</span></div><input type='text'  class=\"ingredients\" id='SUBingredient"+n+"' /></div>";
    $('#SUBingredients1').append(stringa);
	setUpTypeahed();
    }
}
